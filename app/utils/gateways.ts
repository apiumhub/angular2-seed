...
export interface Server {
    ...
    post<T>(resource: string, payload: T): Observable<T>;
}
;
...
export class AxiosGateway implements Server {
    ...
    private request<T>(resource: string, method: string, data: T = null, headers: any = {'X-Custom-Header': 'foobar'}) {
    ...
    post<T>(resource: string, payload: T): Observable<T> {
        return this.request<T>(resource, 'post', payload);
    }
}
